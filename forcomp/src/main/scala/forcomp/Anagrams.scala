...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.groupBy(x => x).map(e => (e._1, e._2.length)).toList.sortBy(x => x)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def combinationsAcc(occ: Occurrences, acc: List[Occurrences]) : List[Occurrences] = {
      occ match {
        case Nil => acc
        case head::tail =>  {
          val r1 = for {e <- acc; i <-(1 to head._2) } yield ((head._1, i)::e).sortBy(x => x)
    ...
    List(List()):::combinationsAcc(occurrences, List())
  }
