...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet) = {
    //way to slow - teacher's example
   //(that incl elem) union  (left union right)
    filterAcc(p => true, that)
  }
  
  def mostRetweeted: Tweet = {
    def max (t: Tweet, ts: TweetSet) ={
      if (ts.isInstanceOf[Empty]) t
      else {
        val m = ts.mostRetweeted
        if ( m.retweets > t.retweets) m else t
      }
    }		
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter{
      t => google.exists(s => t.text.contains(s))
    }
  }

  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter{
      t => apple.exists(s => t.text.contains(s))
    }
  }
